...
const etablissements = companies.map((company) => ({
  value: company.SIREN,
  label: company.nom,
  isEntreprise: company.entreprise,
}));
const etablissementsAffiches = ref([]);
etablissementsAffiches.value = etablissements.filter(
  (company) => company.isEntreprise === true
);

watch(etu, (value) => {
  if (value) {
    etablissementsAffiches.value = etablissements.filter(
      (company) => company.isEntreprise === false
    );
  } else {
    etablissementsAffiches.value = etablissements.filter(
      (company) => company.isEntreprise === true
...
async function enregistrer() {
  ...
  try {
    ...
    const data = await $fetch(
      `http://localhost:8080/PasserellePro/Serv?op=enregistrer_util`,
      {
        method: "POST",
        body: JSON.stringify({
          nom: nom.value,
    ...
    <CardContent>
      <div class="grid gap-4">
        ...
        <div class="grid gap-2">
          ...
          <Popover v-model:open="open">
            <PopoverTrigger as-child>
              ...
              >
                {{ idEtab ? label : "Sélectionner votre établissement..." }}
        ...
        <Button @click="enregistrer" type="submit" class="w-full">
          ...
          <div v-else>Créer un compte</div>
        </Button>
      </div>
      <div class="mt-4 text-center text-sm">
        Vous avez déjà un compte ?
        <a href="/auth/login" class="underline"> Se connecter </a>
      </div>
    </CardContent>
  </Card>
</template>